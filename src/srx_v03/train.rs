//! High-performance Analytical Reversible Backpropagation and AdamW Optimizer Engine for SRXFORMER v03.
//! Implements exact analytical gradients through Monarch Butterfly Unitary factorization,
//! Selective Dynamic Memory Gate, MUSIC Noise Subspace Projection, and Adaptive Epsilon Annealing.
...
/// Gradient storage for all trainable parameters of SrxTransformer v03.
#[derive(Debug, Clone)]
pub struct SrxGrad {
...
/// Pre-allocated workspace for SRX v03 forward activations and backward adjoints.
#[derive(Debug, Clone)]
pub struct SrxTrainWorkspace {
    ...
    pub d_y_raw: Vec<f32>,      // [max_seq_len, d_model]
...
/// Zero-dependency AdamW Optimizer for SrxTransformer v03.
#[derive(Debug, Clone)]
pub struct SrxAdamW {
    pub m: SrxGrad,
    pub v: SrxGrad,
...
pub fn forward_loss(
    ...
    // 3.1. Pillar 1: Selective Memory Gate Computation:
    ...
    // 4. Per-head SRX Recurrence with Monarch Butterfly Unitary & Selective Dynamic Memory Gate
    for h in 0..n_heads {
        ...
        for t in 0..seq_len {
            ...
            // d) Associative Memory accumulation with Selective Dynamic Memory Gate:
            ...
            for row in 0..4 {
                ...
                for col in 0..4 {
                    ...
                    ws.m[m_off + idx] = lambda_val * p_val + gamma_val * (kr * v_raw_h[col]);
                }
            }
...
pub fn backward_loss(
    ...
    // 7. Backward through SRX Attention Recurrence (Monarch Butterfly + Selective Memory Gate)
    for h in 0..n_heads {
        let h_off = h * head_dim;
        let span_theta = 4;
        let span_m = head_dim * head_dim;
        ...
        for t in (0..seq_len).rev() {
            ...
            for row in 0..4 {
                for col in 0..4 {
                    let m_val = ws.m[m_off + row * 4 + col];
                    d_q_rot[row] += d_y_ret[col] * m_val;
                    d_m_acc[row * 4 + col] += ws.q_rot[q_norm_off + row] * d_y_ret[col];
                }
            }

            let q_norm_arr: &[f32; 4] = ws.q_norm[q_norm_off..q_norm_off + 4].try_into().unwrap();

            // Pillar 4: Reversible VJP backward through q_rot Butterfly rotation (forward mode)
            ...
            // Pillar 4: Reversible VJP backward through q_inv Butterfly rotation (inverse mode)
            ...
            for row in 0..4 {
                ...
                for col in 0..4 {
                    let dm_val = d_m_acc[row * 4 + col];
                    d_k_rot[row] += dm_val * v_raw_h[col] * gamma_val;
                    ws.d_v_raw[t * d + h_off + col] += dm_val * kr * gamma_val;

                    // d/d(gamma_t):
                    // M_t = (1 - (1 - gamma_base) * gamma_t) * M_{t-1} + gamma_t * (kr * v)
                    let p_val = if t > 0 { ws.m[pm_off + row * 4 + col] } else { 0.0 };
                    let d_write = kr * v_raw_h[col] - (1.0 - SRX_GAMMA_BASE) * p_val;
                    d_gamma_t += dm_val * d_write;
                }
            }

            // Pillar 1: Backward through Selective Dynamic Memory Gate:
            // gamma_t = sigmoid(s_gate)
            let s_sig = gamma_val * (1.0 - gamma_val);
            let d_s_gate = d_gamma_t * s_sig;

            // Gradient w.r.t b_gamma and W_gamma
            ...
            // M recurrence step to t - 1: d_M_{t-1} = lambda_t * d_M_t
            ...
            // Pillar 4: Reversible VJP backward through k_rot Butterfly rotation (forward mode)
            ...
            // Theta recurrence: d_Theta_{t-1} = d_Theta_t
    ...
    for t in 0..seq_len {
        ...
        for c in 0..d {
            ...
            for k_idx in 0..d {
                let ak = a_tok[k_idx];
                let row_off = c * d + k_idx;
                grad.w_q[row_off] += dq * ak;
                grad.w_k[row_off] += dk * ak;
                grad.w_v[row_off] += dv * ak;

                ws.d_a[t * d + k_idx] += dq * layer.attn.w_q[row_off]
                    + dk * layer.attn.w_k[row_off]
                    + dv * layer.attn.w_v[row_off];
            }
        }
    }
...
/// High-level dataset training method for SRX v03 using analytical BPTT, dynamic epsilon annealing, and AdamW.
pub fn train_dataset(
    ...
    for epoch in 0..epochs {
        ...
        // Smooth quadratic epsilon annealing (Pillar 4):
        // eps(s) = eps_min + (eps_max - eps_min) * (1 - s / S)^2
        let eps_min = 1e-3f32;
        let eps_max = 1.0f32;
...
mod tests {
    ...
    #[test]
    fn test_srx_v03_gradient_check_numerical() {
