//! SRX Attention v02: Super-Resolvent Unitary Givens Attention with MUSIC Subspace Resonance.
//! Enhancements in v02:
//! 1. Hard Gain Clipping: w_clamped = min(w_t, w_max) where w_max = 10.0 (prevents spectral clicks).
//! 2. Post-MUSIC RMSNorm across multi-head retrieved output vector:
//!    y_t = RMSNorm( w_clamped * M_t^T (U_t q_t) )
//! 3. Fast Givens Rotations with Taylor sin/cos eliminating scalar libc bottlenecks.
...
/// Default inference epsilon for Dirac-like peak (v02 uses 1e-3 for stability)
...
/// Super-Resolvent xFormer Attention layer v02.
/// Contains projection matrices W_q, W_k, W_v, W_o (no biases, exact 512 total parameters).
#[derive(Debug, Clone)]
    ...
    pub fn step(
        ...
        for h in 0..n_heads {
            ...
            for col in 0..head_dim {
                ...
                out_h[col] = sum_m * w_clamped;
            }
        }

        // 7. Post-MUSIC RMSNorm:
        // y_t = RMSNorm( w_clamped * M_t^T (U_t q_t) )
        let mut sum_sq = 0.0f32;
        for &val in y_heads.iter() {
            sum_sq += val * val;
        }
        let rms = (sum_sq / d as f32 + 1e-5).sqrt();
        let inv_rms = 1.0 / rms;
        for val in y_heads.iter_mut() {
            *val *= inv_rms;
        }

        // 8. Output projection: W_o y
