...
pub fn forward_loss(
    ...
    for h in 0..n_heads {
        ...
        for t in 0..seq_len {
            ...
            // Dirac-like resonant gain with hard clipping (v02 innovation)
            let w_val = 1.0 / (noise_energy + eps_srx);
            let w_clamped = w_val.min(super::attention::SRX_W_MAX);
    ...
    // Post-MUSIC RMSNorm across multi-head retrieved output vector (v02 innovation)
    for t in 0..seq_len {
        let mut sum_sq = 0.0f32;
        for c in 0..d {
...
pub fn backward_loss(
    ...
    // 6.1. Backward through Post-MUSIC RMSNorm (v02 innovation)
    // y = y_raw / rms, where rms = sqrt(sum(y_raw^2)/d + 1e-5)
    for t in 0..seq_len {
    ...
    for h in 0..n_heads {
        ...
        for t in (0..seq_len).rev() {
            ...
            let d_w = if w_val < super::attention::SRX_W_MAX {
                d_w_clamped
            } else {
                0.0
            };
...
pub fn train_dataset(
    ...
    for epoch in 0..epochs {
        ...
        // Smooth quadratic epsilon annealing (v02 innovation):
        // eps(s) = eps_min + (eps_max - eps_min) * (1 - s / S)^2
        // eps_max = 1.0, eps_min = 1e-3
        let eps_min = 1e-3f32;
        let eps_max = 1.0f32;
