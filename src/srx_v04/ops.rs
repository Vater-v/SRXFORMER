//! Optimized numerical operations for SRXFORMER v04:
...
mod tests {
    ...
    fn test_butterfly_inverse_vjp_gradient_check() {
        let thetas = [-0.6, 1.1, -0.4, 0.9];
        let v = [1.1, 0.2, -1.5, 2.0];
        let d_out = [-0.8, 1.4, -0.3, 0.6];

        let mut d_v = [0.0f32; 4];
        let mut d_thetas = [0.0f32; 4];
        apply_butterfly_4_backward(&v, &thetas, true, &d_out, &mut d_v, &mut d_thetas);
        ...
        for i in 0..4 {
            ...
            let mut out_plus = [0.0f32; 4];
            let mut out_minus = [0.0f32; 4];
            apply_butterfly_4(&v_plus, &thetas, true, &mut out_plus);
            apply_butterfly_4(&v_minus, &thetas, true, &mut out_minus);
            ...
            assert!(
                (d_v[i] - num_grad).abs() < 2e-3,
                "Inverse d_v[{}] gradient mismatch: ana={}, num={}",
                i,
                d_v[i],
                num_grad
            );
        ...
        for i in 0..4 {
            ...
            let mut out_plus = [0.0f32; 4];
            let mut out_minus = [0.0f32; 4];
            apply_butterfly_4(&v, &th_plus, true, &mut out_plus);
            apply_butterfly_4(&v, &th_minus, true, &mut out_minus);
            ...
            assert!(
                ...
                "Inverse d_thetas[{}] gradient mismatch: ana={}, num={}",
                i,
                d_thetas[i],
                num_grad
            );
